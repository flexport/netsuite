  ...
  describe '.get' do
    ...
    before(:each) do
      expect(NetSuite::Actions::Get).to receive(:call)
        .with([NetSuite::Records::VendorCredit, {:external_id => 1}], {})
        .and_return(response)
    end

    context 'when the response is successful' do
      let(:response) { NetSuite::Response.new(:success => true, :body => {:exchange_rate => 100}) }

      it do
        is_expected
          .to be_kind_of(NetSuite::Records::VendorCredit)
          .and have_attributes(:exchange_rate => 100)
      end
    end
